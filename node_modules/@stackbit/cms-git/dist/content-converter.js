...
async function convertDocument(options) {
    const { filePath, fullFilePath, data, getModelByName, assetsConfig, fileIdKey, logger } = options;
    const { type, ...fields } = data;
    if (!type) {
        logger.warn(`File at '${filePath}' is missing the 'type' field. 'type' should map to a model`);
        return null;
    }
    const model = getModelByName(type);
    if (!model) {
        logger.warn(`No model found with name: '${type}'`);
        return null;
    }
