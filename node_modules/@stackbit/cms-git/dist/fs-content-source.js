...
class FileSystemContentSource {
        ...
        for (const contentDir of this.contentDirs) {
            ...
            await (0, utils_1.readFilesFromDirectory)(contentDirPath, this.logger, async (filePath, fullFilePath, data) => {
                if (!data[this.fileIdKey]) {
                    ...
                    this.userLogger.info(`→ Updating file ${filePath} with new id ${documentId}`);
    ...
    async onFilesChange({ updatedFiles }) {
        ...
        for (const filePath of contentFiles) {
            ...
            if (!(await fs_extra_1.default.pathExists(fullFilePath))) {
                ...
                if (existingDocument) {
                    // When a file is renamed, there will be two updated files, one deleted and one added, if their IDs are the same, don't report it as deleted.
                    // The onFilesChange may be called for both files at once, or for each file separately!
                    // When called for both files at once, and the loop already saw the added file, don't add the document to the deleted documents.
                    const addedNewFileWithSameId = documents.find((document) => document.id === existingDocument.id);
                    if (!addedNewFileWithSameId) {
            ...
            // When a file is renamed, there will be two updated files, one deleted and one added, if their IDs are the same, don't report it as deleted.
            // The onFilesChange may be called for both files at once, or for each file separately!
            // When called for both files at once, and the loop already seen the deleted file, remove it from the deleted documents.
        ...
        const models = this.models.map((model) => {
            ...
            if (((0, sdk_1.isPageModel)(updatedModel) || (0, sdk_1.isDataModel)(updatedModel)) && updatedModel.filePath && typeof updatedModel.filePath === 'string') {
                ...
                for (const token of tokens) {
                    ...
                    const fieldType = token === utils_1.momentDateToken ? 'date' : 'slug';
                    if (this.fileIdKey) {
    ...
    async createDocument(options) {
        ...
        for (const fieldName in options.updateOperationFields) {
            ...
            if (modelField && (0, content_converter_1.isVirtualSlug)(modelField.name)) {
    ...
    async updateDocument(options) {
        ...
        const virtualSlugData = Object.entries(document.fields).reduce((result, [fieldName, documentField]) => {
            if ((0, content_converter_1.isVirtualSlug)(fieldName)) {
                const localizedField = stackbitUtils.getLocalizedFieldForLocale(documentField);
                if (localizedField && 'value' in localizedField) {
                    result[(0, content_converter_1.getFieldNameFromVirtualSlug)(fieldName)] = localizedField.value;
                }
            }
            return result;
        }, {});
